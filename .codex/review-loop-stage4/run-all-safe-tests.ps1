...
$runs = @(
    ...
    @{ Name = 'app-identity'; Path = 'tests\Guard.ApplicationIdentity.Tests\bin\Release\net10.0\Guard.ApplicationIdentity.Tests.dll'; Dotnet = $true },
    @{ Name = 'app-control-policy'; Path = 'tests\Guard.AppControlPolicy.Tests\bin\Release\net10.0\Guard.AppControlPolicy.Tests.dll'; Dotnet = $true },
    @{ Name = 'app-control'; Path = 'tests\Guard.ApplicationControl.Tests\bin\Release\net10.0\Guard.ApplicationControl.Tests.dll'; Dotnet = $true },
    @{ Name = 'app-request-protocol'; Path = 'tests\Guard.ApplicationRequestProtocol.Tests\bin\Release\net10.0\Guard.ApplicationRequestProtocol.Tests.dll'; Dotnet = $true },
    @{ Name = 'app-requests'; Path = 'tests\Guard.ApplicationRequests.Tests\bin\Release\net10.0\Guard.ApplicationRequests.Tests.dll'; Dotnet = $true }
)
...
if ($total -ne 232) {
    Write-Error ("Expected 232 passing checks, observed {0}." -f $total)
    exit 1
}
