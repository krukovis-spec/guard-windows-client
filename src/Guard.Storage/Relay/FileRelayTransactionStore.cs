...
namespace Guard.Storage.Relay
{
    ...
    /// below. Initialization publishes the version-zero journal commitment
        ...
        IDisposable
    {
            ...
            try
            {
                    ...
                    File.Exists(_backupFilePath))
                {
                    throw new InvalidOperationException(
                        "Relay state initialization requires absent state and backup files.");
                }
                ...
                var prepared = DecodeEnvelope(envelope);
                if (watermark == null)
                {
                    try
                    {
                        await _watermark.AdvanceToAsync(
                            ...
                            prepared.StateCommitment,
                            cancellationToken).ConfigureAwait(false);
                    }
                    catch (Exception exception)
                    {
                        throw new IOException(
                            "The initial relay journal could not be persisted.",
                            exception);
                    }
                }
                else if (watermark.Version != 0 ||
                    !CryptographicOperations.FixedTimeEquals(
                        watermark.GetStateCommitmentCopy(),
                        prepared.StateCommitment))
                {
                    throw new StateStoreCorruptionException(
                        "An orphaned relay journal does not match the requested initial state.");
                }

                await WriteNewStateFileAsync(
                    envelope,
                    CancellationToken.None).ConfigureAwait(false);
                await VerifyPublishedStateAsync(
                    initialState,
            ...
            RelayTransactionState next)
        {
            ...
            try
            {
                ...
                if (changedRequestCount == 0)
                {
                    if (receipt.Status == CommandReceiptStatus.AlreadyResolved)
                    {
                    ...
                    else if (receipt.Status == CommandReceiptStatus.Rejected)
                    {
                        disposition = RelayApprovalDisposition.Rejected;
                    }
                    else if (receipt.Status == CommandReceiptStatus.Expired)
                    {
                        disposition = RelayApprovalDisposition.Expired;
                    }
                    else
                    {
                        return false;
