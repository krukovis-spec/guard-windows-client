...
- Внешние gates: Hyper-V module и `vmms` есть, но `Get-VM`/`Get-VMHost` возвращают отказ в доступе текущему сеансу. `wrangler whoami` — не авторизован. 2026-09-25 в существующем Cloudflare-аккаунте создан отдельный `guard-relay.voicepaste.workers.dev`: пока только техническая заглушка, которая возвращает HTTP 404; production-код relay не развёрнут. Токен с правом Editor только для `guard-relay` подготовлен, но ещё не выпущен. Тестовая VM по-прежнему требует разрешённого доступа; системную защиту на хосте не запускать.
...
- Для разработки и закрытого семейного пилота используется отдельный Guard-проект на Cloudflare Workers Free с SQLite Durable Objects/D1 и `workers.dev`; Android push использует бесплатный FCM. Guard не переиспользует VoicePaste database/namespace и получает отдельный минимально scoped token.
- Подключение Cloudflare повторяет проверенный маршрут VoicePaste: отдельный токен Guard передаётся Wrangler только через временную `CLOUDFLARE_API_TOKEN`, master copy хранится в Bitwarden, локальная operational copy — через DPAPI CurrentUser с ACL текущего пользователя вне Git; browser OAuth не является основным маршрутом. Проверено 2026-09-25: аккаунт `7135a2335784ee593d8942ef1d79525f` также содержит `voicepaste-api`; созданный `guard-relay` использует тот же account и отдельный Worker. Токен VoicePaste не переиспользовать. Для Guard подготовлен account-owned token `Guard relay deployment` с областью `Specified Workers: guard-relay`, ролью `Individual Workers Editor` и сроком 90 дней; окончательное создание и безопасное сохранение ещё ожидают подтверждения Ивана. Аккаунтный `Workers Admin` затронул бы и VoicePaste и не подходит для постоянного хранения.
