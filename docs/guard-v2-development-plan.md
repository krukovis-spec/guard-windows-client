...
- Выпущен отдельный account-owned токен `Guard relay deployment` без срока действия, `Specified Workers: guard-relay` / `Individual Workers Editor`. Новая PowerShell-сессия получила HTTP 200 для проверки токена и `guard-relay`, HTTP 403 для `voicepaste-api`. Локальная operational copy сохранена через DPAPI CurrentUser с ACL только пользователя вне Git; буфер обмена очищен. Master copy в Bitwarden ещё должен сохранить Иван на открытой странице Cloudflare.
