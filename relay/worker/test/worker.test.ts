...
describe("mailbox authorization and signing intent", () => {
  ...
  it("keeps browser BFF routes fail-closed until real WebAuthn verification is provisioned", async () => {
    const result = await request("/v1/auth/login/options", { method: "POST" });
    expect(result.status).toBe(503);
    await expect(result.json()).resolves.toEqual({ error: "webauthn_bff_not_configured" });
  });
