...
export interface Env { DEVICE_MAILBOX: DurableObjectNamespace; BOOTSTRAP_ADMIN_TOKEN?: string; }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (/^\/v1\/(auth\/(register|login)\/(options|complete)|parent\/(inbox|approval-intents))$/.test(url.pathname)) return fail(503, "webauthn_bff_not_configured");
