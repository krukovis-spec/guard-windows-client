...
- Внешние gates: Hyper-V module и `vmms` есть, но `Get-VM`/`Get-VMHost` возвращают отказ в доступе текущему сеансу. `wrangler whoami` — не авторизован; отдельного Guard deployment нет. Новые ресурсы/аккаунты не создавались, временный анонимный relay не используется. Нужны интерактивный вход Ивана в Cloudflare и разрешённый доступ к тестовой VM; системную защиту на хосте не запускать.
