...
- Подключение Cloudflare повторяет проверенный маршрут VoicePaste: отдельный токен Guard передаётся Wrangler только через временную `CLOUDFLARE_API_TOKEN`, master copy хранится в Bitwarden, локальная operational copy — через DPAPI CurrentUser с ACL текущего пользователя вне Git; browser OAuth не является основным маршрутом. Проверено 2026-09-25: аккаунт `7135a2335784ee593d8942ef1d79525f` также содержит `voicepaste-api`; созданный `guard-relay` использует тот же account и отдельный Worker. Токен VoicePaste не переиспользовать. Иван разрешил выпустить account-owned token `Guard relay deployment` без срока действия, с областью `Specified Workers: guard-relay` и ролью `Individual Workers Editor`; токен ещё не выпущен из-за недоступности браузерного управления. Аккаунтный `Workers Admin` затронул бы и VoicePaste и не подходит для постоянного хранения.
