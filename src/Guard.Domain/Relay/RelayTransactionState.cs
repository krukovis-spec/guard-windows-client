...
namespace Guard.Domain.Relay
{
    ...
    public sealed class RelayTransactionState
    {
            ...
            RelayApprovalTransaction transaction)
        {
            ...
            else if (transaction.Disposition == RelayApprovalDisposition.Rejected ||
                transaction.Disposition == RelayApprovalDisposition.Expired)
            {
                var expectedStatus =
                    transaction.Disposition == RelayApprovalDisposition.Rejected
                        ? CommandReceiptStatus.Rejected
                        : CommandReceiptStatus.Expired;
                if (transaction.PolicyLedgerEntry != null ||
                    transaction.ReconcileIntent != null ||
                    transaction.SignedReceipt.Status != expectedStatus)
                {
                    throw new InvalidOperationException(
                        "A terminal non-decision receipt cannot mutate request or policy state.");
                }
            }
            else
            {
