...
- Какая мобильная платформа родителя выпускается первой и какой общий UI/runtime используется для двух approval-клиентов; криптографическая и biometric-only границы всё равно реализуются нативными адаптерами.
